...
public class App {
	...
	// On crée une nouvelle instance de tableau users
	String users[][] = new String[4][6];
	int userId = -1; // on crée une varialbe qui correspond au 1er [] du tableau users, on met moins
						// -1 pour que la création commence bel et bien à l'index 0
	...
	private void newUser() {
		Scanner sc = new Scanner(System.in);
		//on déclare les variables que l'on va récupérer avec la saisie clavier et utiliser dans le tableau
		String nom,prenom, pays, dateNaiss;
		...
		nom=sc.nextLine();
		users[userId][0]=nom; //le champ prend la valeur de la varialbe nom remplie par l'utilisateur
		
		System.out.println("Quel est votre prénom ?");
		prenom=sc.nextLine();
		users[userId][1]=prenom;
		
		System.out.println("Quel est votre pays de résidence ?");
		pays=sc.nextLine();
		users[userId][2]=pays;
		
		System.out.println("Quel est votre date de naissance ?");
		dateNaiss=sc.nextLine();
		users[userId][3]=dateNaiss;
	...
	private void write() {
		...
		auteur=sc.nextLine();
		messages[msgId][0]=auteur; //le champ prend la valeur de la varialbe nom remplie par l'utilisateur
